...
function locationSpecificLinkScore(
    link: SiteLink,
    clinicCity?: string | null,
) {
    if (!clinicCity) {
...
Deno.serve(
    async (
        ...
        try {
             ...
             */
            let websiteUrl =
                typeof job.website_url ===
                    'string' &&
                    job.website_url.trim()
                    ? stripTrackingParams(
                        job.website_url.trim(),
                    )
                    : null;
             ...
             */
            let pricePage =
                await discoverPriceFromContext(
                    contextHtml,
                    contextUrl,
                    job.clinic_city,
                );
            ...
            if (pricePage) {
                ...
                if (
                    locationPricePage &&
                    locationPricePage.score >=
                    pricePage.score
                ) {
                    const locationPage =
                        await fetchHtml(
                            locationPricePage.url,
                        );
            ...
            if (
                ...
                console.error(
                    'Could not save discovered price source:',
                    updateError,
                );
